...
def loadData():
    images = os.listdir("/home/srishilesh/Srishilesh/Machine learning/Car detection-Srishilesh 22-9-18/Car Datasets/data")
    train_data = []
    train_labels = []
    for image in images:
        if image[-4:] == 'jpeg':
            transformed_image = transform_img(
                io.imread("/home/srishilesh/Srishilesh/Machine learning/Car detection-Srishilesh 22-9-18/Car Datasets/data" + '/' + image))
            train_data.append(transformed_image)
            label_file = image[:-5] + '.txt'
            with open("/home/srishilesh/Srishilesh/Machine learning/Car detection-Srishilesh 22-9-18/Car Datasets/data" + '/' + label_file) as f:
                content = f.readlines()
...
print ("Accuracy score is ",accuracy_score(test_labels, preds))